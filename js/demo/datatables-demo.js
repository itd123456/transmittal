  		...
  		success : function(data)
  		{
  			...
  			for (var i = 0; i < length; i++)
  			{
  				...
  				[
  					...
  					branch,
  					date,
  					'<button style="width:100%" class="btn btn-success" data-toggle="modal" data-target="#deliveredModal" id="edit" name="edit" onclick="delivered('+id+')"><i class="fa fa-check"></i>&nbspMark as Delivered</button><button class="btn btn-info" data-toggle="modal" data-target=".remarksModal" style="width:100%; height:15%" onclick="remark('+id+')"><i class="fa fa-eye"></i>&nbspView</button>'
  				]).draw( false );
  			}
  		}
  	});
...
$('#markDelivered').on('click', function()
{
	...
	{
		type : "POST",
		url : './php/update-trans-status.php',
		data : {id : del},
		dataType : "",
		success : function()
		{
